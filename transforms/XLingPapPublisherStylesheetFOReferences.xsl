...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
   ...
   -->
    <xsl:template match="iso639code">
        <fo:basic-link external-destination="url({normalize-space(.)})">
    ...
    <xsl:template match="url">
        ...
        <fo:basic-link external-destination="url({normalize-space(translate(.,'&#x200b;',''))})">
            <xsl:call-template name="AddAnyLinkAttributes">
                <xsl:with-param name="override" select="$pageLayoutInfo/linkLayout/urlLinkLayout"/>
            </xsl:call-template>
            <!--            <xsl:text>&#x20;</xsl:text>-->
   ...
   -->
    ...
    <xsl:template name="ReportNoPatternMatched">
        <fo:inline background-color="yellow">Sorry, but there is no matching layout for this item in the publisher style sheet.  Please add  (or have someone add) the pattern.</fo:inline>
    </xsl:template>
</xsl:stylesheet>
