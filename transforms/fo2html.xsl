...
<!-- =============================================================== -->
<!-- Changes by hab  -->
<!--  If an image is SVG, have it output <object data="file" type="image/svg+xml"/>  -->
<!-- =============================================================== -->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" exclude-result-prefixes="fo">
   ...
   <xsl:param name="sLocalDirectory" select="'file:///C:/Documents%20and%20Settings/Andy%20Black/My%20Documents/XLingPap/SILEWPTrial'"/>
   <!-- =============================================================== -->
   ...
   <xsl:template match="fo:page-sequence">
      ...
      <!-- Start real drawing -->
      <br/>
      <!-- make an offset before each page sequence -->
      <!-- Header -->
      ...
      <br/>
      <!-- make an offset after each page sequence -->
   </xsl:template>
   <!-- =============================================================== -->
   <!-- fo:block                                                        -->
   <!-- =============================================================== -->
  ...
  <table>&add-style;
   ...
   <xsl:template match="fo:external-graphic">
      ...
      <xsl:choose>
         <xsl:when test="substring-before($cleaned-url, '.svg')!=''">
         &anchor;<object data="{$cleaned-url}" type="image/svg+xml">
               <xsl:apply-templates select="@height|@width|@*[starts-with(name(),'border')]"/>
               <xsl:variable name="sSvgFile">
                  <xsl:value-of select="$sLocalDirectory"/>
                  <xsl:text>/</xsl:text>
                  <xsl:value-of select="$cleaned-url"/>
               </xsl:variable>
               <xsl:variable name="svgFile" select="document($sSvgFile)"/>
            <xsl:if test="$svgFile/@width">
               <xsl:attribute name="width">
                  <xsl:value-of select="$svgFile/@width"/>
               </xsl:attribute>
            </xsl:if>
            <xsl:if test="$svgFile//svg/@height">
               <xsl:attribute name="height">
                  <xsl:value-of select="$svgFile/svg/@height"/>
               </xsl:attribute>
            </xsl:if>
            
            </object>
         </xsl:when>
         <xsl:otherwise>&anchor;<img src="{$cleaned-url}">
               <xsl:apply-templates select="@height|@width|@*[starts-with(name(),'border')]"/>
            </img></xsl:otherwise>
      </xsl:choose>
   </xsl:template>
  ...
  &anchor;<a href="#{@internal-destination}">&add-style;<xsl:apply-templates/></a>
   ...
   <xsl:template match="*[@white-space-collapse='false'
                    ...
                    or @white-space='pre']" mode="check-for-pre">
   ...
   <xsl:template match="@*" mode="convert-to-pixels">
      <xsl:variable name="scaling-factor">
         <xsl:choose>
            ...
            <xsl:when test="contains (., 'em')">16</xsl:when>
            <!-- guess: 1em = 12pt -->
   ...
   <xsl:template match="fo:region-before | fo:region-after">
      ...
      <xsl:variable name="style">
         <xsl:apply-templates select="@*[not (starts-with (name(), 'margin')
                          ...
                          or starts-with (name(), 'padding'))]" mode="collect-style-attributes">
   ...
   <xsl:template match="fo:region-body">
      ...
      <xsl:variable name="style">
         <xsl:apply-templates select="@*[not (starts-with (name(), 'margin')
                          or starts-with (name(), 'space'))]" mode="collect-style-attributes">
            <xsl:with-param name="orientation" select="@reference-orientation"/>
         </xsl:apply-templates>
      </xsl:variable>
      <xsl:if test="string-length($style) &gt; 0">
