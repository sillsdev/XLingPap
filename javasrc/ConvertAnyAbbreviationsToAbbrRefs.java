...
public class ConvertAnyAbbreviationsToAbbrRefs extends RecordableCommand {
	...
	try {
	    ...
	    if (gloss == null || (!"gloss".equals(gloss.name().localPart) && !"item".equals(gloss.name().localPart))) {
		return null;
	    }
	...
	if (abbr.length() > 1) {
		...
		if ((results != null) && (results.length > 0)) {
		    XNode[] resultsRest = getAbbreviations(abbr.substring(1), abbreviations, docView);
