...
<!-- sort without initial "van (der)" and "de" in author names, to follow Unified style sheet for linguistics -->
	<xsl:sort lang="en" select="concat(substring-after(concat('#^',@name), '#^van der '),substring-after(concat('#^',@name), '#^van '),substring-after(concat('#^',@name), '#^de '),@name)" data-type="text"/>
    </xsl:apply-templates>
...
<xsl:template match="refAuthor">
  <xsl:copy>
    <xsl:copy-of select="@*"/>
<!-- convert " and " to " & " within author names, to follow Unified style sheet for linguistics -->
	...
	<xsl:if test="contains(@name, ' and ')">
		<xsl:attribute name="name"><xsl:value-of select="concat(substring-before(@name, ' and '),' &amp; ',substring-after(@name, ' and '))"/>
		</xsl:attribute>
	</xsl:if>
