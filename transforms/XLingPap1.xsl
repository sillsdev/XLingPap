...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//lingPaper">
        <html>
            <head>
                ...
                <xsl:comment> generated by XLingPap1.xsl Version <xsl:value-of select="$sVersion"
                    />&#x20;</xsl:comment>
                <title>
                    <xsl:variable name="sTitle">
                        <xsl:apply-templates
                            select="frontMatter/title/child::node()[name()!='endnote']"/>
                    </xsl:variable>
    ...
    <!--
      ...
      -->
    <xsl:template
        match="date[ancestor::chapterInCollection] | presentedAt[ancestor::chapterInCollection]">
        <div>
            <i>
                <xsl:text>&#xa0;&#xa0;&#xa0;&#xa0;&#xa0;&#xa0;</xsl:text>
                <xsl:apply-templates/>
    ...
    <xsl:template match="contents[not(ancestor::chapterInCollection)]">
        ...
        <div>
            ...
            <ul>
                <xsl:if
                    test="//acknowledgements[parent::frontMatter][not(ancestor::chapterInCollection)]">
                    <li>
                    ...
                    <xsl:for-each select="//chapter | //chapterInCollection">
                        <li>
                                ...
                                <xsl:choose>
                                    <xsl:when test="name()='chapterInCollection'">
                                        <xsl:apply-templates select="frontMatter/title"
                                            mode="showChapterInCollectionTitle"/>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:apply-templates select="secTitle"/>
                                    </xsl:otherwise>
                            ...
                            <ul>
                                ...
                                <xsl:call-template
                                    name="OutputChapterInCollectionBackMatterContents">
                ...
                <xsl:if test="not(//part) and not(//chapter) and not(//chapterInCollection)">
                    <xsl:call-template name="OutputAllSectionTOC">
                        ...
                        <xsl:with-param name="nodesSection1"
                            select="//section1[not(parent::appendix)]"/>
                    </xsl:call-template>
                </xsl:if>
                ...
                <xsl:if
                    test="//acknowledgements[parent::backMatter][not(ancestor::chapterInCollection)]">
                    <li>
                        <a href="#{$sAcknowledgementsID}">
                            <xsl:call-template name="OutputAcknowledgementsLabel"/>
                        </a>
                                        ...
                                        <xsl:variable name="sId">
                                            <xsl:call-template name="GetIdToUse">
                                                <xsl:with-param name="sBaseId"
                                                  select="concat($sPrefaceID,count(preceding-sibling::preface))"
                                                />
                                            </xsl:call-template>
                                        </xsl:variable>
                                        <a name="{$sId}">
    ...
    <xsl:template match="chapter | chapterBeforePart | chapterInCollection">
        <xsl:if
            test="name()='chapterInCollection' and not(preceding-sibling::*[1][name()='frontMatter'][preface]) and not(preceding-sibling::*[1][name()='secTitle'])">
            <hr size="3"/>
        </xsl:if>
        <xsl:call-template name="OutputChapTitle">
   ...
   -->
            ...
            <table>
                <tr>
                    ...
                    <td>
                        ...
                        <xsl:choose>
                            <xsl:when
                                test="name($myFirstChild) = 'exampleHeading' and substring(name(child::*[position()=2]), 1, 4)='list'">
                                <xsl:apply-templates select="exampleHeading" mode="noInitialSpace"/>
                                <table cellpadding="0pt" cellspacing="0pt">
                                    <xsl:apply-templates
                                        select="listInterlinear | listWord | listSingle">
                                        <xsl:with-param name="bListsShareSameCode"
                                            select="$bListsShareSameCode"/>
                                    </xsl:apply-templates>
                                </table>
                            ...
                            <xsl:when
                                test="name($myFirstChild) = 'exampleHeading' and name(child::*[position()=2])='table'">
    ...
    <xsl:template name="OutputISOCodeInExample">
        ...
        <xsl:variable name="firstLangData"
            select="descendant::langData[1] | key('InterlinearReferenceID',interlinearRef/@textref)[1]/descendant::langData[1]"/>
        <xsl:if test="$firstLangData">
            <xsl:variable name="sIsoCode"
                select="key('LanguageID',$firstLangData/@lang)/@ISO639-3Code"/>
    ...
    <xsl:template match="line">
        ...
        <tr>
            ...
            <xsl:if
                test="count(preceding-sibling::line) = 0 and not(parent::lineGroup[preceding-sibling::*[1][name(.)='lineGroup']])">
            ...
            <xsl:choose>
                <xsl:when test="@type">
                    <xsl:element name="div">
                        ...
                        <xsl:choose>
                            <xsl:when
                                test="child::ol | child::ul | child::dl | child::img | child::object | child::br">
                                <xsl:apply-templates/>
                            </xsl:when>
                            <xsl:otherwise>
    ...
    <xsl:template match="listOfTablesShownHere">
        <div>
            <ul>
                <xsl:for-each
                    select="//tablenumbered[not(ancestor::endnote or ancestor::framedUnit)]">
                    <li>
                        <a href="#{@id}">
                            <xsl:call-template name="OutputTableNumberedLabelAndCaption">
                                <xsl:with-param name="bDoBold" select="'N'"/>
            ...
            <xsl:when test="ancestor::chapterInCollection">
                <xsl:call-template name="DoEndnotes">
                    <xsl:with-param name="endnotesToShow">
                        ...
                        <xsl:for-each
                            select="ancestor::chapterInCollection/descendant::interlinearRef">
                            <xsl:for-each select="key('InterlinearReferenceID',@textref)[1]">
                    ...
                    <xsl:with-param name="endnotesToUse"
                        select="ancestor::chapterInCollection/descendant::endnote | ancestor::chapterInCollection/descendant::interlinearRef"
                    />
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="img">
        ...
        <xsl:choose>
            <xsl:when test="substring($sSrc,string-length($sSrc)-3) ='.svg'">
                <embed src="{$sSrc}" type="image/svg+xml"
    ...
    <xsl:template mode="number" match="*">
      ...
      -->
        <xsl:number level="multiple"
            count="section1 | section2 | section3 | section4 | section5 | section6" format="1.1"/>
   ...
   -->
    <xsl:template name="DoInterlinearLineGroup">
        ...
        <table cellpadding="0pt" cellspacing="0pt">
            ...
            <xsl:if
                test="name(../..)='interlinear' or name(../..)='listInterlinear' and name(..)='interlinear' and $iParentPosition!=1">
                <xsl:attribute name="style">
                    <xsl:text>margin-left: 0.1in</xsl:text>
                </xsl:attribute>
                <xsl:if
            ...
            <p style="text-indent:-0.25in;margin-bottom:0in;margin-top:0in">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:if
                            test="substring($author,string-length($author),string-length($author))!='.'"
                            >.</xsl:if>
                        <xsl:text>&#x20;  </xsl:text>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            ...
                            <xsl:if test="collection/multivolumeWork">
                                <i>
                                    <xsl:value-of
                                        select="normalize-space(collection/multivolumeWork)"/>
                                </i>
                                <xsl:text>.&#x20;</xsl:text>
                            </xsl:if>
                            <xsl:choose>
                ...
                <xsl:when test="$originalContext">
                    <xsl:for-each select="$originalContext">
                        ...
                        <xsl:call-template name="DoBookEndnotesLabeling">
                            <xsl:with-param name="originalContext" select="$originalContext"/>
                            <xsl:with-param name="chapterOrAppendixUnit"
                                select="$chapterOrAppendixUnit"/>
                        </xsl:call-template>
                    </xsl:for-each>
                </xsl:when>
    ...
    <xsl:template name="OutputAbbreviationInTable">
        ...
        <tr>
            <td valign="top">
                <xsl:call-template name="HandleColumnWidth">
                    <xsl:with-param name="sWidth"
                        select="normalize-space($abbrsShownHere/@abbrWidth)"/>
                </xsl:call-template>
                <a name="{@id}">
                    <xsl:call-template name="OutputAbbrTerm">
                        <xsl:with-param name="abbr" select="."/>
            ...
            <td valign="top">
                <xsl:call-template name="HandleColumnWidth">
                    <xsl:with-param name="sWidth"
                        select="normalize-space($abbrsShownHere/@equalsWidth)"/>
                </xsl:call-template>
                <xsl:text> = </xsl:text>
            </td>
            <td valign="top">
                <xsl:call-template name="HandleColumnWidth">
                    <xsl:with-param name="sWidth"
                        select="normalize-space($abbrsShownHere/@definitionWidth)"/>
                </xsl:call-template>
                <xsl:call-template name="OutputAbbrDefinition">
                    <xsl:with-param name="abbr" select="."/>
                </xsl:call-template>
   ...
   -->
    <xsl:template name="OutputAbbrTerm">
        ...
        <span>
            ...
            <xsl:if
                test="$abbreviations/@usesmallcaps='yes' or string-length($sFontAttributes) &gt; 0">
                <xsl:attribute name="style">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:if test="$fDoBackgroundColor='Y'">
                    <xsl:choose>
                        <xsl:when
                            test="$fDoStyleAttribute='Y' and string-length(@backgroundcolor) &gt; 0">
                            <xsl:attribute name="style">
                                <xsl:call-template name="OutputBackgroundColor"/>
                            </xsl:attribute>
                        </xsl:when>
